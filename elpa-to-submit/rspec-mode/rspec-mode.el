...
(defun rspec-verify ()
  ...
  (interactive)
  (rspec-run-single-file (rspec-spec-file-for (buffer-file-name)) "--format documentation --drb --profile"))

(defun rspec-verify-single ()
  "Runs the specified example at the point of the current buffer."
  (interactive)
  (rspec-run-single-file (rspec-spec-file-for (buffer-file-name)) "--format documentation --drb --profile" (concat "--line " (number-to-string (line-number-at-pos)))))
...
(defun rspec-run (&rest opts)
  ...
  (compile (concat "bundle exec rake spec SPEC_OPTS=\'" (mapconcat (lambda (x) x) opts " ") "\'") t)
...
(defun rspec-run-single-file (spec-file &rest opts)
  ...
  (compile (concat "bundle exec rspec " spec-file " -c " (mapconcat (lambda (x) x) opts " ")) t)
  (end-of-buffer-other-window 0))
